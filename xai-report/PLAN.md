...
- sl-report: processed data, full-supervision checkpoints for all seven rungs, `pmsm_physics.py` (copied, disclosed).
...
- **Part 4 - Physics-consistency reading.** Do the data-driven models rediscover the LPTN structure: importance of coolant and of the copper-loss proxy `i_d² + i_q²` for temperatures, back-EMF terms for voltage-linked targets. Evidence-based answer, read against the equations.
...
- **H4 (amended 2026-08-15, see `reports/HYPOTHESES.md`):** parameter drift follows material physics. The learned `α_cu` is positive and the learned `α_mag` is negative, signs stable across all 3 seeds, and per-temperature-band refits agree with the learned trends.
...
| `stability.py` | Spearman correlations across levels and seeds, parameter-drift tracking, the three-way LPTN/TNN/PINN parameter cross-check |
...
4. Attribution coverage: importance vectors exist for all seven rungs at the full and 1 % levels.
