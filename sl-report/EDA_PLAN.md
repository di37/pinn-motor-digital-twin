...
| 5. Target integrity | ranges and physical plausibility of all five measured target channels (three modeling targets after the 2026-08-15 trim, the two dropped stator temperatures audited as signals), per-session torque quality, label balance analog = coverage of the operating envelope | temperatures outside [0, 200] °C, torque beyond ±300 Nm, sessions where torque freezes or contradicts `i_q` sign | notebook 01, section 9 |
...
- Script `00a` writes the fingerprint that phase 1 checks against. Notebook 01 runs phases 1-7 on the full file and closes by designing the working sample: 40 stratified sessions, one seeded contiguous block per session, `SAMPLE_TARGET_ROWS = 100,000` (about 10 % of the file).
- Script `00b` draws the sample. Notebook 02 re-runs the audit on the sample, adds phases 8-9, writes the risk register and preprocessing plan, and ends with the modeling readiness verdict, which now includes the sample-adequacy gate decision (section 8).
- Script `00c` runs only after notebook 02's verdict. The train/val/test split is made once the sample-vs-full comparison has passed (G1 carries the plots-and-distributions similarity, G2 and G3 the thermal checks) or, if the gate fired, once the user's decision is recorded. `00c` then freezes the splits and the preprocessing stats and verifies the frozen `HYPOTHESES.md` is unchanged. Confirmatory work starts only after the verdict cell reads ready.
...
## 8. Sample-adequacy gate (pre-registered decision rule, added 2026-08-15)

The 100k block sample is a candidate, not a commitment. Notebook 02 evaluates it using three criteria with thresholds fixed now, before the sample is drawn:

- **G1, envelope:** the sample's speed-torque occupancy overlaps the full file's occupied cells at 90 % or better (Jaccard over a fixed 20 × 20 grid), and every signal's 5th and 95th percentiles shift by less than 5 % of the full-file range.
- **G2, thermal-arc capture:** the median over sampled sessions of (`pm` excursion inside the block divided by `pm` excursion in the whole session) is at least 0.6. The preliminary seeded check measured 0.21, so this is the criterion expected to fail, and the gate exists to make that finding official on the real drawn sample.
